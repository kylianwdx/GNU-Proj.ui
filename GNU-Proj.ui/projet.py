...
class MaFenetre(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def connecteSignaux(self):
        ...
        self.EraseButton.clicked.connect(self.effaceSelection)
        return
    ...
    def effaceSelection(self):
            self.selectionEdit.setText("")
            self.achercher = ""
            self.montreData()
            print("la, il faudrait ajt du texte")
            return
    ...
    def montreData(self):
            ...
            for ligne in self.data[1:]:
                   ok = self.achercher in "".join(ligne)
                   if ok:
